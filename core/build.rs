...
/// Compile `holidays_ru.json` into `HOLIDAYS`/`WORKDAYS` static arrays.
///
/// The JSON holds RF public-holiday and transferred-workday dates (calendar
/// facts, not copyrightable). Anchoring on `CARGO_MANIFEST_DIR` keeps the read
/// correct regardless of the build's working directory.
fn main() {
    let manifest_dir =
