...
|-----------------|---------------------|--------------------------------------------------|
| `core/`         | `laba-core`   | Pure library: API client, normalization, config, timelog, capabilities. No UI. |
| `cli/`          | `laba-cli`    | Thin CLI over core (`laba` binary, JSON by default). |
| `gui/src-tauri/`| `laba-gui`    | Thin Tauri commands over core; window/tray/lifecycle. |
...
- **Quick full build.** `./scripts/gui-build-debug.sh` builds the debug profile
  ...
  binary at `target/debug/laba-gui` runs on the host. Use the release
  command above for a shippable/verified build.

**Verify (host):**

- Rust: `cargo nextest run -p laba-core -p laba-cli`,
...
- `laba-gui` clippy needs the container (same libdbus reason).
